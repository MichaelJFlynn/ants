...
` {
	...
	ld := m.Bfs(n, ANT_1)
	if ld.Loc != s && ld.Loc != m.FromRowCol(1, 0) {
		t.Errorf("bfs doesn't find ants. start %d goal %d or %d found %d", n, s, m.FromRowCol(1, 0), ld.Loc)
	}

	if m.Bfs(n, FOOD).Loc != -1 {
		t.Errorf("bfs doesn't handle not finding item gracefully.")
	}

	/*
		nm := NewMap(20, 20)
		nm.DLE(m.FromRowCol(0, 0), 5)
		//strSteps := nm.StringStep(nm.FromRowCol(0, 0))
		for _, i := range m.Adjacent(m.FromRowCol(0, 0)) {
			if m.StepDirMap[m.FromRowCol(0, 0)][Location(3)].Steps != 1 {
				t.Errorf("DLE incorrect. %d", m.StepDirMap[m.FromRowCol(0, 0)][i.Loc].Steps)
			}
		}
